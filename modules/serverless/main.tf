...
data "archive_file" "crown_jewel_bot_zip" {
  type        = "zip"
  source_dir  = var.crown_jewel_source_dir
  output_path = "${path.module}/crown_jewel_bot.zip"
}
...
resource "google_storage_bucket_object" "crown_jewel_bot_zip" {
  name   = "crown_jewel_bot_${data.archive_file.crown_jewel_bot_zip.output_md5}.zip"
  bucket = google_storage_bucket.cf_source_bucket.name
  source = data.archive_file.crown_jewel_bot_zip.output_path
}
...
resource "google_cloudfunctions2_function" "webhook_remediation" {
  ...
  service_config {
    ...
    environment_variables = {
      ...
      CROWN_JEWEL_BUCKET       = var.crown_jewel_bucket_name
    }
  }
}
...
# ── CROWN JEWEL BOT (Real-time pipeline) ────────────────────────────────

resource "google_cloud_run_service_iam_member" "crown_jewel_trigger_invoker" {
  location = google_cloudfunctions2_function.crown_jewel_bot.location
  service  = google_cloudfunctions2_function.crown_jewel_bot.name
  role     = "roles/run.invoker"
  member   = "serviceAccount:${var.soar_sa_email}"
}

resource "google_cloudfunctions2_function" "crown_jewel_bot" {
  name        = "crown-jewel-bot"
  location    = var.region
  project     = var.project_id
  description = "Real-time SOC bot for Crown Jewel access"
  ...
  build_config {
    ...
    source {
      storage_source {
        bucket = google_storage_bucket.cf_source_bucket.name
        object = google_storage_bucket_object.crown_jewel_bot_zip.name
      }
    }
  }
  ...
  event_trigger {
    trigger_region        = var.region
    event_type            = "google.cloud.pubsub.topic.v1.messagePublished"
    pubsub_topic          = var.crown_jewels_topic_id
    retry_policy          = "RETRY_POLICY_DO_NOT_RETRY"
    service_account_email = var.soar_sa_email
  }
}
